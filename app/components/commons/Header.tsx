...
// Type pour les items de menu, incluant une action optionnelle
type MenuItem = {
  href: string;
  label: string;
...
export function Header() {
  ...
  // Effet de scroll pour faire descendre le header
  useEffect(() => {
    const handleScroll = () => {
      const scrollPosition = window.scrollY;
      setIsScrolled(scrollPosition > 20); // Déclenche après 20px de scroll
    };
  ...
  return (
    <>
      <header className={`fixed left-0 right-0 z-30 flex justify-center p-0 transition-all duration-500 ease-out ${
        isScrolled 
          ? '-top-[1.6rem] sm:-top-[1.35rem] md:-top-[1.55rem]' 
          : '-top-[2rem] sm:-top-[2rem] md:-top-[2rem]'
      }`}>

        <div className="relative flex items-center justify-between w-full max-w-7xl px-2 sm:px-2 md:px-4 z-50">
          <TooltipProvider>
            <Dock 
              data-dock="true"
              ...
              className={cn(
                ...
                "h-10 md:h-16", // Hauteur fixe pour toutes les tailles d'écran
              ...
              <DockIcon className="mr-0.5 sm:mr-1 md:mr-2">
                <Link href="/" className="relative z-60">
                  <div className="relative w-6 h-6 sm:w-6 sm:h-6 md:w-11 md:h-11 rounded-md sm:rounded-lg md:rounded-xl overflow-hidden bg-gradient-to-br from-blue-500/40 via-purple-500/40 to-pink-500/40 backdrop-blur-sm border border-zinc-600/60 flex items-center justify-center group hover:scale-110 transition-all duration-300 ease-in-out shadow-lg hover:shadow-blue-500/25">
                    <div className="absolute inset-0 bg-gradient-to-br from-blue-400/20 via-purple-400/20 to-pink-400/20 animate-pulse group-hover:animate-none"></div>
                    <div className="absolute inset-0 bg-gradient-to-br from-blue-300/15 via-purple-300/15 to-pink-300/15 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    <Image src="/logo2.svg" alt="Logo" width={14} height={14} className="relative z-10 group-hover:scale-110 transition-transform duration-300 sm:w-4 sm:h-4 md:w-6 md:h-6" />
                  </div>
                </Link>
              </DockIcon>
              ...
              <DockIcon className="mx-0.5 sm:mx-1 md:mx-1">
                <Separator orientation="vertical" className="h-4 sm:h-4 md:h-6 bg-zinc-700/60 shadow-sm" />
              </DockIcon>
                ...
                <DockIcon key={item.label} className="mx-0.5">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <item.icon className="h-3.5 w-3.5 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4 group-hover:scale-110 transition-transform duration-300" />
                        <span className="sr-only">{item.label}</span>
                      </Link>
                    </TooltipTrigger>
                    <TooltipContent className="bg-zinc-900/95 border-zinc-800 text-zinc-200 font-medium">
                      <p>{item.label}</p>
                    </TooltipContent>
                  </Tooltip>
                </DockIcon>
              ...
              <DockIcon className="ml-0.5">
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <MailIcon className="h-3.5 w-3.5 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4 group-hover:scale-110 transition-transform duration-300" />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent className="bg-zinc-900/95 border-zinc-800 text-zinc-200 font-medium">
